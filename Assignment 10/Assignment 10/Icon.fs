...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Prim1 (ope, e1) -> // 11.8
        eval e1 (fun v -> fun econt1 ->
            ...
            | ("even", Int i) -> if i&&&1 = 0 then cont (Int i) econt1 else econt1 ()
            | ("mult", Int i) -> // 11.8.4
                let rec aux a b cont =
                    aux a (a+b) (fun v -> fun _ -> cont (Int b) econt1)
                aux i i (fun v -> fun _ -> cont (Int i) econt1)
            | _ -> Str "unknown prim1")
            econt
    | And(e1, e2) -> 
