...
printfn "Exercise 4.1"
open Absyn;;
open Fun;;
let res = run (Prim("+", CstI 5, CstI 7)) |> printfn "%A"


open Parse;;
fromString "5+7" |> printfn "%A"
fromString "let y = 7 in y + 2 end" |> printfn "%A"
fromString "let f x = x + 7 in f 2 end" |> printfn "%A"

open ParseAndRun;;
run (fromString "5+7") |> printfn "%A"
run (fromString "let y = 7 in y + 2 end") |> printfn "%A"
run (fromString "let f x = x + 7 in f 2 end") |> printfn "%A"

//exercise 4.2
printfn "Exercise 4.2"
...
// exercise 4.4
printfn "exercise 4.4"
fromString "let pow x n = if n=0 then 1 else x * pow x (n-1) in pow 3 8 end" |> run |> printfn "%A"
fromString "
    let max2 a b = 
...
// exercise 4.5
printfn "exercise 4.5"

fromString "true && false" |> run |> printfn "%A"
fromString "true && true" |> run |> printfn "%A"
fromString "true || false" |> run |> printfn "%A"
fromString "false || false" |> run |> printfn "%A"
